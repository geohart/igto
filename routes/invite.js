...
nodemailer.SMTP = {
	    host: secret.mail_host, // required
	    use_authentication: true, // optional, false by default
	    user: secret.mail_user, // used only when use_authentication is true 
	    pass: secret.mail_pass  // used only when use_authentication is true
}
...
function emailInvite(find, me, them, self){
  ...
  if(find.mode == 0){
    ...
    if (me.name){
      themSubject = me.name + ' (' + me.contact + ') wants to know where you are';
      themBody = 'Hi there!\r\n\r\n' + me.name + ' (' + me.contact + ')';
    }else{
      themSubject = me.contact + ' wants to know where you are';
      themBody = 'Hi there!\r\n\r\n' + me.contact;
    }
  ...
  if(self){
    ...
    if(find.mode == 0){
      ...
      meBody = meBody + 'Click below to view: \r\n\r\n' + __dirname + '/share?id=' + find._id + '&code=' + me.code + '\r\n\r\n';
    ...
    if (them.name){
      meSubject = 'You requested ' + them.name + '\'s (' + them.contact + ') location';
      meBody = meBody + 'Hi there!\r\n\r\nYou asked ' + them.name +' (' + them.contact + ') to share his/her location with you.\r\n\r\n';
    }else{
